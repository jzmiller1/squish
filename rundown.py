...
nlcd_data = [('data\\nlcd\\nlcd_2001_landcover_2011_edition_2014_10_10\\nlcd_2001_landcover_2011_edition_2014_10_10.img', '2001'),
             ('data\\nlcd\\nlcd_2006_landcover_2011_edition_2014_10_10\\nlcd_2006_landcover_2011_edition_2014_10_10.img', '2006'),
             ('data\\nlcd\\nlcd_2011_landcover_2011_edition_2014_10_10\\nlcd_2011_landcover_2011_edition_2014_10_10.img', '2011')]


for data, year in nlcd_data:
    clipped_image = 'clipped_{}.tif'.format(year)
    clipped_image_path = os.path.join(BASE_DIR, 'data\\nlcd\\output\\' + clipped_image)
    os.system("gdalwarp -te 603361 889606 1472518 1407064 {} {}".format(os.path.join(BASE_DIR, data),
                                                                        clipped_image_path,
                                                                        ))
    output_base_filename = 'tile_{}'.format(year)
    dset = gdal.Open(clipped_image_path)
    width = dset.RasterXSize
    height = dset.RasterYSize

    for i in range(0, width, TILESIZE):
        for j in range(0, height, TILESIZE):
            # get tile width and height coords
            w = min(i+TILESIZE, width) - i
            h = min(j+TILESIZE, height) - j
    
            # create the srcwin_command
            output_file = "_".join([output_base_filename, str(i), str(j)]) + '.tif'
            tile_coords = ", ".join([str(coord) for coord in i, j, w, h])
            srcwin_command = " ".join([tile_coords, clipped_image_path, os.path.join('data\\nlcd\\output\\', output_file)])
    
            # do the work
            gdaltranString = "gdal_translate -of GTIFF -srcwin " + srcwin_command
            os.system(gdaltranString)



tiles = glob.glob(os.path.join(BASE_DIR, 'data\\nlcd\\output\\*tile*.tif'))
...
for i, tile in enumerate(tiles):
    ...
    raster2pgsql_command = " ".join([raster2pgsql_path,
                                     ...
                                     "-t 100x100",
                                     "-s 96630",
                                     os.path.join(BASE_DIR, tile),
                                     "{}.nlcdtiles".format(DATA_SCHEMA)])
    cursor.execute(subprocess.check_output(raster2pgsql_command, shell=True))
    conn.commit()
